...
public sealed class GartenDokumentePage : FooterContentPage
{
        ...
        ParzelleSelectionState parzelleSelection)
    {
        ...
        _header = new Label { Text = "Dokumente", FontSize = 24, FontAttributes = FontAttributes.Bold };
        ...
        _list = new CollectionView
        {
            ItemTemplate = new DataTemplate(() =>
            {
                ...
                var meta = new Label { Opacity = 0.8, FontSize = 12 };
                meta.SetBinding(Label.TextProperty, new Binding(nameof(DocumentInfo.UpdatedAt), stringFormat: "{0:g}"));
        ...
        _list.SelectionChanged += OnDocumentSelected;
    ...
    private async Task EnsureInitializedCoreAsync()
    {
        ...
        await LoadDokumenteAsync(_loadedParzelleId.Value);
    }
    ...
    private async Task LoadDokumenteAsync(int parzelleId)
    {
        SetBusy(true);
        try
        {
            ...
            var docs = await _supabaseService.GetParzelleDokumenteAsync(parzelleId);
            var items = (docs ?? new List<DocumentInfo>())
                .Where(x => x != null)
            ...
            if (items.Count == 0)
                _status.Text = "Keine Dokumente vorhanden.";
        }
        catch (Exception ex)
        {
            _status.Text = ex.Message;
    ...
    private async void OnDocumentSelected(object? sender, SelectionChangedEventArgs e)
    {
        ...
        if (e.CurrentSelection?.FirstOrDefault() is not DocumentInfo doc)
            return;
        ...
        try
        {
            ...
            {
                await DisplayAlert("Fehler", "Dokument konnte nicht geöffnet werden.", "OK");
                return;
            }
