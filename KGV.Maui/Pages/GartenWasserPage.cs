...
public sealed class GartenWasserPage : ContentPage
{
        ...
        ParzelleSelectionState parzelleSelection)
    {
        ...
        _header = new Label { Text = "Wasser", FontSize = 24, FontAttributes = FontAttributes.Bold };
    ...
    private async Task LoadAblesungenAsync(int parzelleId)
    {
        ...
        try
        {
            var list = await _supabaseService.GetWasserAblesungenAsync(parzelleId);
    ...
    private async void OnAddAblesungClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            var zaehler = await _supabaseService.GetActiveWasserzaehlerAsync(parzelleId, DateTime.Today);
            if (zaehler == null)
            {
                await DisplayAlert("Hinweis", "Kein aktiver Wasserzähler vorhanden. Bitte ggf. zuerst einen Zähler einbauen/tauschen.", "OK");
                return;
            }
            ...
            var ok = await _supabaseService.AddAblesungAsync(ZaehlerTypWasser, zaehler.Id, DateTime.Today, stand, fotoPfad: null);
            if (!ok)
            {
    ...
    private async void OnSwapZaehlerClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            var existing = await _supabaseService.GetActiveWasserzaehlerAsync(parzelleId, DateTime.Today);
            if (existing != null)
            {
                var outOk = await _supabaseService.SetWasserzaehlerAusgebautAmAsync(existing.Id, DateTime.Today);
                if (!outOk)
                {
            ...
            var addOk = await _supabaseService.AddWasserzaehlerAsync(parzelleId, newNr, eichDatum.Date, DateTime.Today);
            if (!addOk)
            {
